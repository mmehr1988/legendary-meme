  ...
  fetch(oneCallApi)
    ...
    .then(function (data) {
      if (recentSearchUL.children.length > 0) {
        ...
        forecastLiEl[0].innerHTML = `Current Temperature: ${Math.round(data.current.temp)} &#176F`;
        forecastLiEl[1].innerHTML = `Feels Like: ${Math.round(data.current.feels_like)} &#176F`;
        forecastLiEl[2].innerHTML = `Condition: ${data.current.weather[0].main}`;
        forecastLiEl[3].innerHTML = `Humidity: ${data.current.humidity} &#37`;
        forecastLiEl[4].innerHTML = `High: ${Math.round(data.daily[0].temp.max)} &#176F`;
        forecastLiEl[5].innerHTML = `Low: ${Math.round(data.daily[0].temp.min)} &#176F`;
        forecastLiEl[6].innerHTML = `Wind Speed: ${data.current.wind_speed} mph`;
        ...
        for (let i = 0; i < fiveDayDate.length; i++) {
          fiveDaySection[i].children[0].innerHTML = moment()
            .add(i + 1, 'days')
            .format('M/D/YYYY');
          fiveDaySection[i].children[1].innerHTML = `Day: ${Math.round(data.daily[i].temp.day)} &#176F`;
          fiveDaySection[i].children[2].innerHTML = `High: ${Math.round(data.daily[i].temp.max)} &#176F`;
          fiveDaySection[i].children[3].innerHTML = `Low: ${Math.round(data.daily[i].temp.min)} &#176F`;
